...
void GameScene::Draw(void) {
	...
	if (m_score < 10) {
		GUI::DrawTextShaded<FontID::FACTORY>("ENTI CRUSH LVL 1",
	...
	else if (m_score >= 10 && m_score < 30) {
		GUI::DrawTextShaded<FontID::FACTORY>("ENTI CRUSH LVL 2",
		{ W.GetWidth() >> 1, int(W.GetHeight()*.1f), 1, 1 },
		{ 190, 0, 160 }, { 50, 200, 230 }); // Render score that will be different when updated
		GUI::DrawTextBlended<FontID::CANDY>("Score: " + std::to_string(m_score),
		{ W.GetWidth() >> 1, int(W.GetHeight()*.9f), 1, 1 },
	...
	else if (m_score >= 30 && m_score < 50) {
		GUI::DrawTextShaded<FontID::FACTORY>("ENTI CRUSH LVL 3",
		{ W.GetWidth() >> 1, int(W.GetHeight()*.1f), 1, 1 },
		{ 190, 0, 160 }, { 50, 200, 230 }); // Render score that will be different when updated
		GUI::DrawTextBlended<FontID::CANDY>("Score: " + std::to_string(m_score),
		{ W.GetWidth() >> 1, int(W.GetHeight()*.9f), 1, 1 },
